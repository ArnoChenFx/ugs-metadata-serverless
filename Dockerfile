...
#
# UGS Metadata Server —— 容器镜像（默认纯本地 SQLite 模式）
#
# 构建：
...
#   docker compose up -d
#
# 切换成 PostgreSQL：设置 DB_DRIVER=postgres 与 DATABASE_URL 即可，镜像本身无需改动。

ARG DENO_VERSION=2.9.7

# ---------------------------------------------------------------------------
# 第一阶段：预热第三方依赖（hono / postgres）
#
...
COPY --chown=deno:deno supabase ./supabase
...
RUN deno cache \
  supabase/functions/ugs-metadata/index.ts \
...
# 运行时配置
# ---------------------------------------------------------------------------
# 默认即纯本地 SQLite 模式，数据落在 /data 卷里，容器重建也不会丢数据。
...
# 健康检查依赖 /health（会真正 ping 一次数据库）。
# 注意：本命令用的是 exec 数组形式，不经过 shell，因此基础镜像里没有 curl/wget 也没问题。
